  ...
  def sifter_btn_click(self, **event_args):
    ...
    for kpi in RSATickets:
        ...
        if kpi_created.year == self.year.selected_value and kpi_created.month == 5:
            ...
            if kpi.status == "Open":
                openSifters = openSifters +1
            elif kpi.status == "Reopened":
                reopenedSifters = reopenedSifters + 1
            elif kpi.status == "Follow Up":
                followupSifters = followupSifters + 1
            elif kpi.status == "Resolved":
                resolvedSifters = resolvedSifters + 1
            elif kpi.status == "Closed":
                closedSifters = closedSifters + 1
            else: 
